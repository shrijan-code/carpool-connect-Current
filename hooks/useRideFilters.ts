...
import { Ride, Delivery, Location } from '@/types';
...
export interface DeliveryFilterOptions {
  ...
  minPrice?: number;
  maxPrice?: number;
  packageSize?: 'small' | 'medium' | 'large' | 'extra_large' | 'all';
  userLocation?: { latitude: number; longitude: number };
}
...
/**
 * Filter deliveries based on provided filters
 */
export function filterDeliveries(deliveries: Delivery[], filters: DeliveryFilterOptions): Delivery[] {
  return deliveries.filter(delivery => {
    const pickupLocation = delivery.pickupLocation;
    const dropoffLocation = delivery.dropoffLocation;
    
    if (!pickupLocation || !dropoffLocation) return false;
    
    // Text-based location filters (check both pickup and dropoff)
    const locationFilters = {
      city: filters.city,
      suburb: filters.suburb,
      postcode: filters.postcode
    ...
    const hasLocationFilter = filters.city || filters.suburb || filters.postcode;
    if (hasLocationFilter) {
      const pickupMatches = matchesLocationFilters(pickupLocation, locationFilters);
      const dropoffMatches = matchesLocationFilters(dropoffLocation, locationFilters);
      if (!pickupMatches && !dropoffMatches) return false;
    }
    
    // Distance filter (if user location is provided)
    // Check if user is within distance of either pickup or dropoff location
    if (filters.maxDistance && filters.userLocation) {
      const withinDistance = isRideWithinDistance(
        pickupLocation, 
        dropoffLocation, 
    ...
    // Price filters
    const priceInDollars = delivery.priceCents / 100;
    if (filters.minPrice && priceInDollars < filters.minPrice) return false;
    if (filters.maxPrice && priceInDollars > filters.maxPrice) return false;
    
    // Package size filter
    if (filters.packageSize && filters.packageSize !== 'all' && delivery.packageSize !== filters.packageSize) {
      return false;
    }
...
/**
 * Hook for managing delivery filters
 */
export function useDeliveryFilters(initialDeliveries: Delivery[] = []) {
  const [filters, setFilters] = useState<DeliveryFilterOptions>({});
  const [deliveries, setDeliveries] = useState<Delivery[]>(initialDeliveries);
  
  // Update deliveries when initialDeliveries changes - use ref to prevent infinite loops
  const prevInitialDeliveriesRef = useRef<Delivery[]>([]);
  const initialDeliveriesStringified = JSON.stringify(initialDeliveries.map(d => d.id));
  
  useEffect(() => {
    const prevStringified = JSON.stringify(prevInitialDeliveriesRef.current.map(d => d.id));
    if (initialDeliveriesStringified !== prevStringified) {
      prevInitialDeliveriesRef.current = initialDeliveries;
      setDeliveries(initialDeliveries);
    }
  }, [initialDeliveriesStringified, initialDeliveries]);
  
  const filteredDeliveries = useMemo(() => {
    console.log('Filtering deliveries:', deliveries.length, 'with filters:', filters);
    const filtered = filterDeliveries(deliveries, filters);
    console.log('Filtered results:', filtered.length, 'deliveries');
    return filtered;
  }, [deliveries, filters]);
  
  const updateFilters = useCallback((newFilters: Partial<DeliveryFilterOptions>) => {
    console.log('Updating delivery filters:', newFilters);
    setFilters(prev => {
      const updated = { ...prev, ...newFilters };
      console.log('New delivery filter state:', updated);
      return updated;
    });
  }, []);
  
  const clearFilters = useCallback(() => {
    console.log('Clearing all delivery filters');
    setFilters({});
  }, []);
  
  const updateDeliveries = useCallback((newDeliveries: Delivery[]) => {
    console.log('Updating deliveries:', newDeliveries.length);
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    console.log('Active delivery filters count:', count, 'from filters:', filters);
    return count;
  }, [filters]);
